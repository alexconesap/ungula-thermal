        ...
        class FanTachometer {
           public:
            ...
            const FanChannelState& getChannelState(uint8_t channel) const {
                ...
                if (channel >= N) return emptyState;
        ...
        class FanController {
           public:
            ...
            FanController(const FanTachConfig& tachCfg, const FanOutputConfig& outCfg)
                : tachometer_(tachCfg), pwmCalculator_(outCfg), samplePeriodMs_(tachCfg.samplePeriodMs), lastUpdateMs_(0) {
            }
            ...
            bool update(uint32_t nowMs, int speedCommandPercent) {
                if (nowMs - lastUpdateMs_ < samplePeriodMs_) return false;
                lastUpdateMs_ = nowMs;
            ...
            bool isAnyFanConnected() const {
                for (uint8_t i = 0; i < N; ++i) {
                    if (tachometer_.getChannelState(i).connected) return true;
                }
                return false;
            }
            ...
            bool areAllFansConnected() const {
                for (uint8_t i = 0; i < N; ++i) {
                    if (!tachometer_.getChannelState(i).connected) return false;
                }
                return true;
            }
